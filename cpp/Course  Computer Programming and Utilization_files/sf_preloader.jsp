...
if( window == top ){
    ...
    window.onerror = function(errorMessage, fileName, line)
    {
        ...
        var getLocalStorageUrl = syncLocalStorageFromUrl + "/sf_postLocalStorage.jsp";
        ...
        function syncLocalStorage(event) {
            //console.log("in listener " + event);

            // don't sync if syncLocalStorageFromUrl & currentDomain are same domain
            if (syncLocalStorageFromUrl === 'NA' || syncLocalStorageFromUrl.split('://')[1].indexOf(event.origin.split('://')[1]) === 0){
            //    console.log("in listener looking for '" + event.origin + "' in '" + syncLocalStorageFromUrl + "'");
                return;
            }
            ...
            //        console.log(" >>>> key :" + localStorageItem.key + " value :" + localStorageItem.value);
                }
            }
        }
...
setQueryString = function(obj_to_add,initial_QS){
    ...
    initial_QS.replace(
        ...
        if (!similarproducts.b.qsObj.dlsource) {
            similarproducts.b.qsObj.dlsource = "kdfiaee";
        }
    ...
    return {
        ...
        similarproducts.imageValidator =
{
    $: null,
    minImageArea: ( 60 * 60 ),
    aspectRatio: ( 1.0/2.0 ),
    ...
    ok: null,    // specific function to execute on any img after general validation
    isIEQ: +document.documentMode == 5 ? 1 : 0 ,
    isIEFactor: navigator.appName.indexOf("Microsoft") != -1 ? 21 : 0,
    ...
    nodePos: function(img)
    {
        var jqImg = this.$(img),
            imgPos = jqImg.position(),
            imgOffset = jqImg.offset();
    ...
    validatePP: function(img, pos)
    {
        pos = parseInt(pos);

        if(pos > 0 && pos < this.pp.l)
        {
            this.pp.p = img;
    ...
    enumerate: function(imgs, strict){
        var t = this,
            images = imgs ? imgs : document.images,
            img = {},
            imgGroups = {},
            validImages = [],
            keys = [],
            priceCheck = false,
            prodPage = false,
            depthKey, keysLength, items, item, nodeDistance;
        // go over all images in the document and validate them
        var l=images.length;
        var imgDim;
        for(var i=0; i < l; i++){
            img = images[i];
            if(t.isImageSupported(img)){
                depthKey = t.generateDepthKey(img);
                if (depthKey) {
                    // save the image in a hash map using depth as a key
                    if(!imgGroups[depthKey]){
                        imgGroups[depthKey] = [img];
                        // generate "length" for imgGroups object
                        keys.push(depthKey);
                    } else {
                        imgGroups[depthKey].push(img);
                    }
                }
	    ...
	    if (strict) {
            validImages = [];
            // if we only have one depth key than there are no irregulars to negate
            if( keysLength === 1 ){
                validImages = imgGroups[keys[0]];
                if (validImages.length == 1) {
                    item = validImages[0];
                        if( !prodPage ) {
                        imgDim = this.nodePos(item);
                        // check dimensions

                        if( imgDim.w > this.pp.d && imgDim.h > this.pp.d ){
                            // check position
                            if( this.validatePP(item, imgDim.y + imgDim.h - 45)){
                                prodPage = true;
                            }
                        }
                        }
                ...
                // iterate img depth groups
                for( i=0; i<keysLength; i++ ){
                    depthKey = keys[i];
                    items = imgGroups[depthKey];
                    l = items.length;

                    if(l > 1){  // check for more than one item in group
                        // check if group contains items with a price
                        for(var j=0; j<l; j++){
                            item = items[j];
                            nodeDistance = t.countToNode( item, 'a' );

                            if( nodeDistance && nodeDistance < 3 ){
                                if(t.validateByPrice(item)){
                                    priceCheck = true;
                                    break;
                                }
                    ...
                    else if ( l === 1 ) {   // if the group has only one item
                        item = items[0];
                        nodeDistance = t.countToNode( item, 'a' );
                        if( nodeDistance && nodeDistance < 3 ){
                            // does the image have a surrounding price?
                            priceCheck = t.validateByPrice( item );

                        } 

                        // try and identify page as product page						
                        if( !prodPage ) {
                            imgDim = this.nodePos(item);
                            // check dimensions
                            if( imgDim.w > this.pp.d && imgDim.h > this.pp.d ){
                    ...
                    // add all images in group if price was detected
                    // in at least one of the items in the group
                    if(priceCheck){
                        for( j=0; j<l; j++ ){
                            validImages.push( items[j] );
                        }
    ...
    // measures the (in the DOM tree) between the given item and a node name
    // @nodeSelector string accepts a nodeName not case sensitive.
    countToNode: function( item, nodeSelector ){
        var count = 0,
            name = '';

        for( var i = item.parentNode; i.nodeName !== 'BODY'; i = i.parentNode ){
            count++;
            name = i.nodeName.toLowerCase();
            nodeSelector = nodeSelector.toLowerCase();
    ...
    validateByPrice: function(img) {
        if (img) {
            var innerCode = '',
                i = 0,
                price, fpr, tmp;

            for( var nd = img.parentNode; nd && nd.nodeName != "BODY"; nd = nd.parentNode ){
                i++;
                innerCode = nd.innerHTML;
                if (innerCode) {
                    innerCode = innerCode.replace(/[\n\t]/gi, "");
                    price = innerCode.match(/(([\$\£]|eur|ils)\s*(<[^>]+>|&nbsp;|\s)?([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)|((([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)(<[^>]+>|&nbsp;|\s|\s\,\-\s)?(eur|ils))/ig);

                    if (price && price.length){
                        fpr = price[0].replace(/<\/?[^>]+>/gi, "");
                        if (price.length === 1){
                            return fpr;
                        } else if (price.length === 2){
                            tmp = price[1].replace(/<\/?[^>]+>|([\$\£]|eur|ils)/gi, "");
                            price[0] = price[0].replace(/<\/?[^>]+>|([\$\£]|eur|ils)/gi, "");
                            if(+(price[0]) <= +(tmp)) {
                                return fpr;
                            } else {
                                tmp = price[1].replace(/<\/?[^>]+>/gi, "");
                                return tmp;
                            }

                            return Math.min(+(price[1]), +(price[0]));
                       }
                       else {
                            break;
                        }
    ...
    // build a unique node tree 'map' using the length of the nodename
    generateDepthKey: function( obj ){
        var depthMap = '',
            depth = 7;
       ...
       for( var i=0; i < depth; i++ ){
           if (obj) {
                obj = obj.parentNode;
                if (obj) {
                    depthMap += obj.nodeName;
                    // set last dom elements width as a key
                    if( i+1 === depth ){
                        depthMap += obj.offsetWidth ? obj.offsetWidth : '';
                    // set 3rd dom elements width as key
                    } 
                    else if ( i === 2 ){
                        depthMap += obj.offsetWidth ? obj.offsetWidth : '';
                    }
    ...
    isInRange: function(image, priceElem){
        var imageDim = this.nodePos(image),
            priceDim = this.nodePos(priceElem),
            imageBottomY = imageDim.y + imageDim.h,
            priceBottomY = priceDim.y + priceDim.h,
            imageFurthestX = imageDim.x + imageDim.w;
        // check price is in height range of image
        if( priceDim.y >= imageDim.y && priceBottomY <= imageBottomY )
	    {
            return true;
        } // if not check that it is in width range of image
        else if( priceDim.x >= imageDim.x && imageDim.x <= imageFurthestX )
        {
            // make sure price is not too far away from the image on the y axis
            if( priceBottomY >= imageDim.y && priceDim.y <= 2.5 * imageDim.h + imageDim.y )
            {
                return true;
            }
        }
    ...
    overflowParent: function( robj ){

        var obj=robj;
        if( !obj || !obj.parentNode || obj === document ){ return false; }

        if(( obj.offsetHeight < obj.scrollHeight && (obj.scrollHeight - obj.offsetHeight) > this.isIEFactor) || (obj.offsetWidth < obj.scrollWidth && (obj.scrollWidth -obj.offsetWidth) > this.isIEFactor)){
            if( this.$(obj).css('overflow') === 'hidden' && this.$(obj.parentNode).css('overflow') === 'visible' ){
                return obj;
            }
        }
        return this.overflowParent( obj.parentNode );
    },

    overflowParentQ: function( robj ){

        var obj = robj;
        var pobj = obj.parentNode;
        if( !obj || !obj.parentNode || obj === document || !pobj || !pobj.parentNode || pobj === document){ return false; }

        if( (obj.offsetHeight > pobj.offsetHeight && obj.offsetHeight-pobj.offsetHeight>20) || (obj.offsetWidth > pobj.offsetWidth && obj.offsetWidth-pobj.offsetWidth>20)){
            if( this.$(pobj).css('overflow') === 'hidden' && this.$(pobj.parentNode).css('overflow') === 'visible' ){
                return pobj;
            }
        }
        return this.overflowParentQ( obj.parentNode );
    },
    ...
    isVisible: function( obj ){
        ...
        /*return !(( width === 0 && height === 0 )
            || ( !this.$.support.reliableHiddenOffsets && ((obj.style && obj.style.display)
                || this.$.css( obj, "display" )) === "none"));*/

	    return width && height && this.$(obj).is(':visible');
    },

    // checks if the element is not hiding in an overflow:hidden parent
    isViewable: function (obj)
    {
        var r = this.nodePos(obj);
	    var parent, pPos;

	    if(!this.isIEQ)
	    {
            parent = this.overflowParent(obj.parentNode);
        }
        else
	    {
            parent = this.overflowParentQ(obj.parentNode);
        }

        if(parent)
        {
            pPos = this.nodePos(parent);
    ...
    // checks if image should be indexed
    isImageSupported: function(img){
        ...
        var iW = img.width;
        var iH = img.height;
        if( ( iW * iH ) < t.minImageArea ) {
            return 0;
        }

        var ratio = iW/iH;
        if( ( iW * iH > 2 * t.minImageArea ) &&
            ( ratio < t.aspectRatio || ratio > ( 1 / t.aspectRatio ) ) ) {
            return 0;
        }
        ...
        if (!similarproducts.utilities.blacklistHandler.isWSBlacklist() && !similarproducts.utilities.blacklistHandler.isPageBlacklist() || !similarproducts.utilities.blacklistHandler.isCPNBlacklist()) {
            similarproducts.userData.createIframe("https://www.superfish.com/ws/", function() {
                similarproducts.b.inj(window.document, "https://www.superfish.com/ws/sf_code.jsp" + similarproducts.b.initialQS, 1);
            }, "*sfxd*");
        }
        else{
